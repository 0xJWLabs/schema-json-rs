...
impl<'a> Container<'a> {
    pub fn from_ast(item: &'a syn::DeriveInput) -> syn::Result<Container<'a>> {
        ...
        ctxt.check()
            .map(|_| result.expect("from_ast set not errors on Ctxt, so should have returned Ok"))
    }

    pub fn transparent_field(&'_ self) -> Option<&'_ Field> {
...
impl Field<'_> {
    ...
    pub fn add_mutators(&self, mutators: &mut Vec<TokenStream>) {
        ...
        if self.serde_attrs.skip_deserializing() {
            mutators.push(quote! {
                schema_jsonrs::_private::insert_metadata_property(&mut #SCHEMA, "readOnly", true);
            });
        }
        if self.serde_attrs.skip_serializing() {
            mutators.push(quote! {
                schema_jsonrs::_private::insert_metadata_property(&mut #SCHEMA, "writeOnly", true);
            });
        }
    }
