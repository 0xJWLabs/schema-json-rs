...
#[proc_macro_derive(JsonSchema, attributes(schema_jsonrs, serde, validate, garde))]
...
#[proc_macro_derive(JsonSchema_repr, attributes(schema_jsonrs, serde))]
...
fn derive_json_schema(mut input: syn::DeriveInput, repr: bool) -> syn::Result<TokenStream> {
    ...
    if let Some(transparent_field) = cont.transparent_field() {
        ...
        return Ok(quote! {
            const _: () = {
                ...
                impl #impl_generics schema_jsonrs::JsonSchema for #type_name #ty_generics #where_clause {
                    fn always_inline_schema() -> bool {
                        <#ty as schema_jsonrs::JsonSchema>::always_inline_schema()
                    }

                    fn schema_name() -> schema_jsonrs::_private::alloc::borrow::Cow<'static, str> {
                        <#ty as schema_jsonrs::JsonSchema>::schema_name()
                    }

                    fn schema_id() -> schema_jsonrs::_private::alloc::borrow::Cow<'static, str> {
                        <#ty as schema_jsonrs::JsonSchema>::schema_id()
                    }
                    ...
                    fn _schema_jsonrs_private_non_optional_json_schema(#GENERATOR: &mut schema_jsonrs::SchemaGenerator) -> schema_jsonrs::Schema {
                    ...
                    fn _schema_jsonrs_private_is_option() -> bool {
                        <#ty as schema_jsonrs::JsonSchema>::_schema_jsonrs_private_is_option()
                    }
                };
            };
        });
        ...
        || (cont.attrs.is_renamed && !schema_base_name.contains('{'))
    {
        (
            quote! {
                schema_jsonrs::_private::alloc::borrow::Cow::Borrowed(#schema_base_name)
            },
            quote! {
        ...
        (
            quote! {
                schema_jsonrs::_private::alloc::borrow::Cow::Owned(
                    schema_jsonrs::_private::alloc::format!(
                        #schema_name_fmt
                        #(,#type_params=#type_params::schema_name())*
                        #(,#const_params=schema_jsonrs::_private::alloc::string::ToString::to_string(&#const_params))*)
                )
            },
            quote! {
                schema_jsonrs::_private::alloc::borrow::Cow::Owned(
        ...
        (
            quote! {
                schema_jsonrs::_private::alloc::borrow::Cow::Owned(
                    schema_jsonrs::_private::alloc::format!(#schema_name_fmt #(,#type_params::schema_name())* #(,#const_params)*)
                )
            },
            quote! {
                schema_jsonrs::_private::alloc::borrow::Cow::Owned(
    ...
    if let Some(bounds) = cont.serde_attrs.ser_bound() {
        ...
        for param in &mut cont.generics.params {
            if let syn::GenericParam::Type(ref mut type_param) = *param {
                type_param.bounds.push(parse_quote!(schema_jsonrs::JsonSchema));
            }
        }
    }
}
