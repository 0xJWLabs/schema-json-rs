...
pub fn parse_length_or_range(outer_meta: Meta, cx: &AttrCtxt) -> Result<LengthOrRange, ()> {
    ...
    for nested_meta in parse_nested_meta(outer_meta, cx)? {
        match path_str(nested_meta.path()).as_str() {
            ...
            unknown => {
                if cx.attr_type == "schema_jsonrs" {
                    cx.error_spanned_by(
                        nested_meta,
                        format_args!(
                            "unknown item in schema_jsonrs {outer_name} attribute: `{unknown}`",
                        ),
                    );
                }
            }
...
pub fn parse_schema_jsonrs_regex(outer_meta: Meta, cx: &AttrCtxt) -> Result<Expr, ()> {
    ...
    for nested_meta in parse_nested_meta(outer_meta.clone(), cx)? {
        match path_str(nested_meta.path()).as_str() {
            ...
            "path" => {
                cx.error_spanned_by(nested_meta, "`path` is not supported in `schema_jsonrs(regex(...))` attribute - use `schema_jsonrs(regex(pattern = ...))` instead")
            },
            unknown => {
                cx.error_spanned_by(
                    nested_meta,
                    format_args!("unknown item in schema_jsonrs `regex` attribute: `{unknown}`"),
                );
            }
        }
    }
    ...
    pattern.ok_or_else(|| {
        cx.error_spanned_by(
            outer_meta,
            "`schema_jsonrs(regex(...))` attribute requires `pattern = ...`",
        )
    })
}
...
pub fn parse_validate_regex(outer_meta: Meta, cx: &AttrCtxt) -> Result<Expr, ()> {
    ...
    for nested_meta in parse_nested_meta(outer_meta.clone(), cx)? {
        match path_str(nested_meta.path()).as_str() {
            ...
            "pattern" => {
                cx.error_spanned_by(nested_meta, "`pattern` is not supported in `validate(regex(...))` attribute - use either `validate(regex(path = ...))` or `schema_jsonrs(regex(pattern = ...))` instead")
            },
            _ => {
...
pub fn parse_contains(outer_meta: Meta, cx: &AttrCtxt) -> Result<Expr, ()> {
    ...
    let nested_meta_or_expr = match cx.attr_type {
        ...
        "schema_jsonrs" => parse_meta_list_with(&outer_meta, cx, ContainsFormat::parse),
        wat => {
            unreachable!(
                "Unexpected attr type `{wat}` for `contains` item. This is a bug in schema_jsonrs, please raise an issue!"
            )
        }
    }?;
            ...
            unknown => {
                if cx.attr_type == "schema_jsonrs" {
                    cx.error_spanned_by(
                        nested_meta,
                        format_args!(
                            "unknown item in schema_jsonrs `contains` attribute: `{unknown}`"
                        ),
                    );
                }
            }
