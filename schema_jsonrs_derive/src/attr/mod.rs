...
pub use schema_jsonrs_to_serde::process_serde_attrs;
...
impl CommonAttrs {
    fn populate(
        &mut self,
        attrs: &[Attribute],
        schema_jsonrs_cx: &mut AttrCtxt,
        serde_cx: &mut AttrCtxt,
    ) {
        self.process_attr(schema_jsonrs_cx);
                        ...
                        if lit_str.parse::<Path>().is_ok() {
                            ...
                            cx.error_spanned_by(&expr, format_args!(
                                "`example` value must be an expression, and string literals that may be interpreted as function paths are currently disallowed to avoid migration errors \
                                 (this restriction may be relaxed in a future version of schema_jsonrs).\n\
                                If you want to use the result of a function, use `#[schema_jsonrs(example = {lit_str_value}())]`.\n\
                                Or to use the string literal value, use `#[schema_jsonrs(example = &\"{lit_str_value}\")]`."));
                        }
                    }
                    ...
                    }) = &expr
                    {
                        if lit_str.parse::<Expr>().is_ok() {
                            cx.error_spanned_by(
                                &expr,
                                format_args!(
                                    "Expected a `fn(&mut Schema)` or other value implementing `schema_jsonrs::transform::Transform`, found `&str`.\nDid you mean `#[schema_jsonrs(transform = {})]`?",
                                    lit_str.value()
                                ),
                            )
                        }
    ...
    pub fn add_mutators(&self, mutators: &mut Vec<TokenStream>) {
        ...
        if let Some(doc) = &self.doc {
            if title.is_none() || description.is_none() {
                mutators.push(quote!{
                    const title_and_description: (&str, &str) = schema_jsonrs::_private::get_title_and_description(#doc);
                });
        ...
        if let Some(title) = title {
            mutators.push(quote! {
                schema_jsonrs::_private::insert_metadata_property_if_nonempty(&mut #SCHEMA, "title", #title);
            });
        }
        if let Some(description) = description {
            mutators.push(quote! {
                schema_jsonrs::_private::insert_metadata_property_if_nonempty(&mut #SCHEMA, "description", #description);
            });
        }

        if self.deprecated {
            mutators.push(quote! {
                schema_jsonrs::_private::insert_metadata_property(&mut #SCHEMA, "deprecated", true);
            });
        }
        ...
        if !self.examples.is_empty() {
            let examples = self.examples.iter().map(|eg| {
                quote! {
                    schema_jsonrs::_private::serde_jsonc2::value::to_value(#eg)
                }
            });
            mutators.push(quote! {
                schema_jsonrs::_private::insert_metadata_property(&mut #SCHEMA, "examples", schema_jsonrs::_private::serde_jsonc2::Value::Array([#(#examples),*].into_iter().flatten().collect()));
            });
        }

        for (k, v) in &self.extensions {
            mutators.push(quote! {
                schema_jsonrs::_private::insert_metadata_property(&mut #SCHEMA, #k, schema_jsonrs::_private::serde_jsonc2::jsonc!(#v));
            });
        }

        for transform in &self.transforms {
            mutators.push(quote! {
                schema_jsonrs::transform::Transform::transform(&mut #transform, &mut #SCHEMA);
            });
        }
    }
}
...
impl FieldAttrs {
    ...
    fn populate(&mut self, attrs: &[Attribute], cx: &Ctxt) {
        let schema_jsonrs_cx = &mut AttrCtxt::new(cx, attrs, "schema_jsonrs");
        ...
        self.common.populate(attrs, schema_jsonrs_cx, serde_cx);
        self.validation
            .populate(schema_jsonrs_cx, validate_cx, garde_cx);
        self.process_attr(schema_jsonrs_cx);
        self.process_attr(serde_cx);
    }
    ...
    fn process_meta(&mut self, meta: Meta, meta_name: &str, cx: &AttrCtxt) -> Option<Meta> {
        match meta_name {
            ...
            "schema_with" if cx.attr_type == "schema_jsonrs" => match self.with {
...
impl<'a> AttrCtxt<'a> {
    ...
    pub fn mutual_exclusive_error(&self, meta: &Meta, other_attr: &str) {
        if self.attr_type == "schema_jsonrs" {
            self.error_spanned_by(
                meta,
                format_args!(
                    "schema_jsonrs attribute cannot contain both `{}` and `{}`",
                    path_str(meta.path()),
                    other_attr,
                ),
            );
    ...
    pub fn duplicate_error(&self, meta: &Meta) {
        if self.attr_type == "schema_jsonrs" {
            self.error_spanned_by(
                meta,
                format_args!(
                    "duplicate schema_jsonrs attribute item `{}`",
                    path_str(meta.path())
                ),
            );
        }
        ...
        if self.attr_type == "schema_jsonrs" {
            for unhandled_meta in self.metas.iter().filter(|m| !is_known_serde_keyword(m)) {
                self.error_spanned_by(
                    unhandled_meta.path(),
                    format_args!(
                        "unknown schema_jsonrs attribute `{}`",
...
fn is_known_serde_keyword(meta: &Meta) -> bool {
    let known_keywords = schema_jsonrs_to_serde::SERDE_KEYWORDS;
    meta.path()
        .get_ident()
