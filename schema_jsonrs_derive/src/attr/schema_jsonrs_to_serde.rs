...
// List of keywords that can appear in #[serde(...)]/#[schema_jsonrs(...)] attributes which we want
...
pub(crate) static SERDE_KEYWORDS: &[&str] = &[
    ...
    // schema_jsonrs attr.
    "bound",
    // Special cases - `with`/`serialize_with` are passed to serde but not copied from schema_jsonrs
    ...
    // serialized. We also check the `with` value on schema_jsonrs/serde attrs e.g. to support deriving
...
// If a struct/variant/field has any #[schema_jsonrs] attributes, then create copies of them
...
fn process_attrs(ctxt: &Ctxt, attrs: &mut Vec<Attribute>) {
    ...
    *attrs = other_attrs;
    ...
    // Copy appropriate #[schema_jsonrs(...)] attributes to #[serde(...)] attributes
    let (mut serde_meta, mut schema_jsonrs_meta_names): (Vec<_>, HashSet<_>) =
        get_meta_items(attrs, "schema_jsonrs", ctxt)
            .into_iter()
            .filter_map(|meta| {
    ...
    if schema_jsonrs_meta_names.contains("skip") {
        schema_jsonrs_meta_names.insert("skip_serializing".to_string());
        schema_jsonrs_meta_names.insert("skip_deserializing".to_string());
    }

    // Re-add #[serde(...)] attributes that weren't overridden by #[schema_jsonrs(...)] attributes
    for meta in get_meta_items(&serde_attrs, "serde", ctxt) {
        if let Some(i) = get_meta_ident(&meta) {
            if !schema_jsonrs_meta_names.contains(&i)
...
mod tests {
    ...
    fn test_process_serde_attrs() {
        let mut input: DeriveInput = parse_quote! {
            ...
            #[schema_jsonrs(rename = "overriden", another_unknown_word)]
            #[misc]
            struct MyStruct {
                /// blah blah blah
                ...
                #[schema_jsonrs(with = "with", bound = "bound")]
                field2: i32,
                #[schema_jsonrs(skip)]
