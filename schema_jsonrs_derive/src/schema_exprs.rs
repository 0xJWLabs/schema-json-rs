...
pub fn expr_for_repr(cont: &Container) -> Result<SchemaExpr, syn::Error> {
    ...
    let mut schema_expr = SchemaExpr::from(quote!({
        let mut map = schema_jsonrs::_private::serde_jsonc2::Map::new();
        map.insert("type".into(), "integer".into());
        map.insert(
            "enum".into(),
            schema_jsonrs::_private::serde_jsonc2::Value::Array({
                let mut enum_values = schema_jsonrs::_private::alloc::vec::Vec::new();
                #(enum_values.push((#enum_ident::#variant_idents as #repr_type).into());)*
                enum_values
            }),
        );
        schema_jsonrs::Schema::from(map)
    }));
...
fn expr_for_field(field: &Field, is_internal_tagged_enum_newtype: bool) -> SchemaExpr {
    ...
    let schema_expr = if field.attrs.validation.required {
        quote_spanned! {span=>
            <#ty as schema_jsonrs::JsonSchema>::_schema_jsonrs_private_non_optional_json_schema(#GENERATOR)
        }
    } else if is_internal_tagged_enum_newtype {
        quote_spanned! {span=>
            schema_jsonrs::_private::json_schema_for_internally_tagged_enum_newtype_variant::<#ty>(#GENERATOR)
        }
    } else {
        quote_spanned! {span=>
...
fn type_for_schema(with_attr: &WithAttr) -> (syn::Type, Option<TokenStream>) {
    match with_attr {
        ...
        WithAttr::Function(fun) => {
            let ty_name = syn::Ident::new("_SchemaJsonrsSchemaWithFunction", Span::call_site());
            let fn_name = fun.segments.last().unwrap().ident.to_string();

            let type_def = quote_spanned! {fun.span()=>
                struct #ty_name;

                impl schema_jsonrs::JsonSchema for #ty_name {
                    fn always_inline_schema() -> bool {
                        true
                    }

                    fn schema_name() -> schema_jsonrs::_private::alloc::borrow::Cow<'static, str> {
                        schema_jsonrs::_private::alloc::borrow::Cow::Borrowed(#fn_name)
                    }

                    fn schema_id() -> schema_jsonrs::_private::alloc::borrow::Cow<'static, str> {
                        schema_jsonrs::_private::alloc::borrow::Cow::Borrowed(::core::concat!(
                            "_SchemaJsonrsSchemaWithFunction/",
                            ::core::module_path!(),
                            "/",
                            #fn_name
                        ))
                    ...
                    fn json_schema(generator: &mut schema_jsonrs::SchemaGenerator) -> schema_jsonrs::Schema {
                        #fun(generator)
                    }
                }
            };
...
fn expr_for_enum(variants: &[Variant], cattrs: &serde_attr::Container) -> SchemaExpr {
    if variants.is_empty() {
        return quote!(schema_jsonrs::Schema::from(false)).into();
    }
    ...
    schemas.extend(complex_variants.into_iter().map(|variant| {
        ...
        let mut schema_expr =
            SchemaExpr::from(if variant.is_unit() && variant.attrs.with.is_none() {
                quote! {
                    schema_jsonrs::_private::new_unit_enum_variant(#name)
                }
            } else {
                let sub_schema = expr_for_untagged_enum_variant(variant, deny_unknown_fields);
                quote! {
                    schema_jsonrs::_private::new_externally_tagged_enum_variant(#name, #sub_schema)
                }
            });
    ...
    let variant_schemas = variants
        .map(|variant| {
            ...
            schema_expr.mutators.push(quote!(
                schema_jsonrs::_private::apply_internal_enum_variant_tag(&mut #SCHEMA, #tag_name, #name, #deny_unknown_fields);
            ));
...
fn expr_for_adjacent_tagged_enum<'a>(
    ...
    let schemas = variants
        .map(|variant| {
            ...
            let mut outer_schema = SchemaExpr::from(quote!(schema_jsonrs::json_schema!({
                "type": "object",
                "properties": {
                    #tag_name: (#tag_schema),
...
fn expr_for_internal_tagged_enum_variant(
    ...
    if let Some(with_attr) = &variant.attrs.with {
        let (ty, type_def) = type_for_schema(with_attr);
        let mut schema_expr = SchemaExpr::from(quote_spanned! {variant.original.span()=>
            <#ty as schema_jsonrs::JsonSchema>::json_schema(#GENERATOR)
        });
...
fn expr_for_tuple_struct(fields: &[Field]) -> SchemaExpr {
    ...
    quote!({
        let mut prefix_items = schema_jsonrs::_private::alloc::vec::Vec::new();
        #(#fields)*
        let len = schema_jsonrs::_private::serde_jsonc2::Value::from(prefix_items.len());
...
fn expr_for_struct(
        ...
        .map(|field| {
            if field.serde_attrs.flatten() {
                ...
                let mut schema_expr = SchemaExpr::from(quote_spanned! {ty.span()=>
                    schema_jsonrs::_private::json_schema_for_flatten::<#ty>(#GENERATOR, #required)
                });

                schema_expr.definitions.extend(type_def);

                field.with_contract_check(quote! {
                    schema_jsonrs::_private::flatten(&mut #SCHEMA, #schema_expr);
                })
            } else {
                let name = field.name();
                let (ty, type_def) = type_for_field_schema(field);
                ...
                let is_optional = if has_skip_serialize_if && has_default {
                    ...
                    quote!(if #GENERATOR.contract().is_serialize() {
                        ...
                        #has_default || (!#required_attr && <#ty as schema_jsonrs::JsonSchema>::_schema_jsonrs_private_is_option())
                    })
                };
                ...
                if let Some(default) = field_default_expr(field, set_container_default.is_some()) {
                    schema_expr.mutators.push(quote! {
                        #default.and_then(|d| schema_jsonrs::_schema_jsonrs_maybe_to_value!(d))
                            .map(|d| schema_jsonrs::_private::insert_metadata_property(&mut #SCHEMA, "default", d));
                    })
                }
                ...
                field.with_contract_check(quote!({
                    #type_def
                    schema_jsonrs::_private::insert_object_property(&mut #SCHEMA, #name, #is_optional, #schema_expr);
                }))
            }
            })
        .collect();
    ...
    SchemaExpr {
        ...
        creator: quote!(schema_jsonrs::json_schema!({
            "type": "object",
            #set_additional_properties
        })),
        mutators: properties,
...
fn field_default_expr(field: &Field, container_has_default: bool) -> Option<TokenStream> {
    ...
    Some(if let Some(ser_with) = field.serde_attrs.serialize_with() {
        quote! {
            {
                struct _SchemaJsonrsDefaultSerialize<T>(T);

                impl serde::Serialize for _SchemaJsonrsDefaultSerialize<#ty>
                {
                ...
                #default_expr.map(|d| _SchemaJsonrsDefaultSerialize(d))
            }
        }
    } else {
        default_expr
