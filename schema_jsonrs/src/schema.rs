...
impl core::convert::TryFrom<Value> for Schema {
    type Error = serde_jsonc2::Error;

    fn try_from(value: Value) -> serde_jsonc2::Result<Schema> {
        Schema::validate(&value)?;
        Ok(Schema(value))
    }
}
...
impl<'a> core::convert::TryFrom<&'a mut Value> for &'a mut Schema {
    ...
    fn try_from(value: &mut Value) -> serde_jsonc2::Result<&mut Schema> {
        Schema::validate(value)?;
