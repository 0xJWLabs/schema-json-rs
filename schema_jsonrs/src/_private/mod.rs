...
use serde::Serialize;
use serde_jsonc2::{jsonc, map::Entry, Map, Value};

mod regex_syntax;
mod rustdoc;

pub extern crate alloc;
pub extern crate serde_jsonc2;
...
pub fn json_schema_for_flatten<T: ?Sized + JsonSchema>(
    ...
    let mut schema = T::_schema_jsonrs_private_non_optional_json_schema(generator);

    if T::_schema_jsonrs_private_is_option() && !required {
...
#[doc(hidden)]
#[macro_export]
macro_rules! _schema_jsonrs_maybe_to_value {
    ($expression:expr) => {{
...
impl<T: Serialize> MaybeSerializeWrapper<T> {
    pub fn maybe_to_value(self) -> Option<Value> {
        serde_jsonc2::value::to_value(self.0).ok()
    }
}
